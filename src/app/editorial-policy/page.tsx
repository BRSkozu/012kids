...
export default function EditorialPolicyPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <h1 className="text-3xl text-[var(--color-foreground)] mb-2" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700 }}>編集方針</h1>
      <p className="text-[var(--color-foreground-muted)] mb-8">
        012.kidsが情報まとめサイトとして、透明性と信頼性を確保するために定めた編集ポリシーです。
      </p>

      {/* Site nature clarification */}
      <div className="bg-[var(--color-warm-cream)] border border-[var(--color-paper-edge)] rounded-xl p-6 mb-8">
        <h2 className="text-lg text-[var(--color-primary-dark)] mb-2" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700 }}>当サイトの位置づけ</h2>
      ...
      <div className="space-y-8">
        {/* Principle 1 */}
        <section className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-paper-edge)] p-6">
          <h2 className="text-lg text-[var(--color-foreground)] mb-3 flex items-center gap-2" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700 }}>
      ...
      <section className="mt-12">
        <h2 className="text-xl text-[var(--color-foreground)] mb-4" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700 }}>品質スコアリング</h2>
        <p className="text-[var(--color-foreground-soft)] text-sm mb-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={item.label} className="flex items-start gap-4">
                <span className="text-sm font-bold text-[var(--color-primary-dark)] bg-[var(--color-warm-cream)] px-2 py-1 rounded shrink-0">
                  {item.weight}
                </span>
                <div>
                  <h3 className="text-sm font-bold text-[var(--color-foreground)]">{item.label}</h3>
                  <p className="text-xs text-[var(--color-foreground-muted)]">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-4 border-t border-[var(--color-paper-edge)] text-sm text-[var(--color-foreground-muted)]">
      ...
      {/* Contact */}
      <div className="mt-12 text-center bg-[var(--color-warm-cream)] rounded-xl p-6">
        <p className="text-sm text-[var(--color-foreground-soft)] mb-3">
